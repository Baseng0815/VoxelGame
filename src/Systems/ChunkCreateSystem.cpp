...
void ChunkCreateSystem::handlePlayerMoved(const EntityMovedEvent &e) {
  ...
  for (auto entity : view) {
    ...
    if (std::abs(newX - chunk.chunkX) > Configuration::CHUNK_PRELOAD_SIZE ||
      ...
      if (!std::count(m_destructionQueue.begin(), m_destructionQueue.end(),
                      entity)) {
  ...
  for (int x = newX + Configuration::CHUNK_PRELOAD_SIZE;
       x >= newX - (int)Configuration::CHUNK_PRELOAD_SIZE; x--) {
    for (int z = newZ + Configuration::CHUNK_PRELOAD_SIZE;
         z >= newZ - (int)Configuration::CHUNK_PRELOAD_SIZE; z--) {
      glm::vec2 pos{x, z};
      if (std::count(m_loadedChunks.begin(), m_loadedChunks.end(), pos) == 0) {
        entt::entity entity = m_registry.create();

        const ChunkComponent &chunk = m_registry.emplace<ChunkComponent>(
            entity, new std::shared_mutex{}, x, z, new Geometry{},
            new Geometry{}, new Geometry{});
        m_registry.emplace<TransformationComponent>(
            entity, glm::vec3{x * Configuration::CHUNK_SIZE, 0,
                              z * Configuration::CHUNK_SIZE});
        m_registry.emplace<MultiMeshRenderComponent>(
            entity, std::vector<MeshRenderComponent>{
                        {ResourceManager::getResource<Material>(
                             MATERIAL_CHUNK_BLOCKS_CULLED),
                         chunk.geometryCulled},
                        {ResourceManager::getResource<Material>(
                             MATERIAL_CHUNK_BLOCKS_NON_CULLED),
                         chunk.geometryNonCulled},
                        {ResourceManager::getResource<Material>(MATERIAL_WATER),
                         chunk.geometryTransparent, 1}});
...
GenerationData ChunkCreateSystem::updateChunkBlocks(entt::entity entity,
...
ChunkGeometryData
ChunkCreateSystem::updateChunkVertices(entt::entity entity,
  ...
  try {
    ...
    std::cerr << "WARNING: chunk buffer preallocation failed: " << e.what()
              << std::endl;
  }
  ...
  for (int x = 0; x < Configuration::CHUNK_SIZE; x++) {
    for (int y = 0; y < Configuration::CHUNK_HEIGHT; y++) {
      for (int z = 0; z < Configuration::CHUNK_SIZE; z++) {
        ...
        // use shared_lock for shared read access and unique_lock for unique
        // write access
        ...
        if (block.type < BlockId::PLANE_GRASS) {
          // negative X
          if (x > 0) {
            const Block &other = chunk.getBlock(x - 1, y, z);
            if (other.isTransparent() && block.type != other.type)
              draw[0] = true;
          } else {
            draw[0] = true;
          }
          ...
          // negative Y
          if (y > 0) {
            const Block &other = chunk.getBlock(x, y - 1, z);
            if (other.isTransparent() && block.type != other.type)
              draw[2] = true;
          } else {
            draw[2] = true;
          ...
          // negative Z
          if (z > 0) {
            const Block &other = chunk.getBlock(x, y, z - 1);
            if (other.isTransparent() && block.type != other.type)
              draw[4] = true;
          } else {
            draw[4] = true;
          ...
          if (block.isTransparent()) {
            ...
            for (size_t face = 0; face < 6; face++) {
              if (draw[face]) {
                for (size_t vi = 0; vi < 4; vi++) {
                  ...
                  geometryData.verticesTransparent.emplace_back(
                      Vertex{glm::vec3{ref[0], ref[1], ref[2]} + blockPosition,
                             glm::vec3{ref[3], ref[4], ref[5]},
                             glm::vec2{blockUVs[ref[6]][ref[7]]}});
                }
                faceCountPerPass++;
              }
            }
            ...
            for (int i = 0; i < faceCountPerPass; i++) {
              ...
              for (size_t j = 0; j < 6; j++) {
                ...
                geometryData.indicesTransparent.emplace_back(faceCount * 4 +
                                                             offsets[j]);
              }
            }
            ...
            for (size_t face = 0; face < 6; face++) {
              if (draw[face]) {
                for (size_t vi = 0; vi < 4; vi++) {
                  const auto &ref = blockGeo[face * 4 + vi];
                  geometryData.verticesCulled.emplace_back(
                      Vertex{glm::vec3{ref[0], ref[1], ref[2]} + blockPosition,
                             glm::vec3{ref[3], ref[4], ref[5]},
                             glm::vec2{blockUVs[ref[6]][ref[7]]}});
                }
            ...
            for (int i = 0; i < faceCountPerPass; i++) {
              ...
              for (size_t j = 0; j < 6; j++) {
                constexpr int offsets[] = {0, 1, 2, 0, 3, 1};
                geometryData.indicesCulled.emplace_back(faceCount * 4 +
                                                        offsets[j]);
              }
            }
          }
        ...
        // special blocks using other geometry which are not culled
          ...
          for (size_t vi = 0; vi < 8; vi++) {
            ...
            geometryData.verticesNonCulled.emplace_back(
                Vertex{glm::vec3{ref[0], ref[1], ref[2]} + blockPosition,
                       glm::vec3{ref[3], ref[4], ref[5]}, uvs[ref[6]]});
          }
          ...
          for (int i = 0; i < 2; i++) {
            ...
            for (size_t j = 0; j < 6; j++) {
              ...
              geometryData.indicesNonCulled.emplace_back(faceCount * 4 +
                                                         offsets[j]);
            }
          }
        }
...
void ChunkCreateSystem::updateChunkBuffers(
    Geometry *geometry, const std::vector<unsigned int> &indices,
...
void ChunkCreateSystem::deleteChunks() {
  ...
  while (it != m_destructionQueue.end()) {
    ...
    if (!chunk.threadActiveOnSelf && chunk.blocks) {
      ...
      m_loadedChunks.erase(
          std::remove(m_loadedChunks.begin(), m_loadedChunks.end(), pos),
          m_loadedChunks.end());
...
void ChunkCreateSystem::processBlocks() {
  ...
  while (itGen != m_generationFutures.end()) {
    if (itGen->valid() &&
        itGen->wait_for(std::chrono::seconds(0)) == std::future_status::ready) {
      GenerationData generationData = itGen->get();
      ChunkComponent &chunk =
          m_registry.get<ChunkComponent>(generationData.entity);
      ...
      World::addChunk(generationData.entity,
                      glm::vec2{chunk.chunkX, chunk.chunkZ});
...
void ChunkCreateSystem::processVertices() {
  ...
  while (itGeo != m_geometryFutures.end()) {
    if (itGeo->valid() &&
        itGeo->wait_for(std::chrono::seconds(0)) == std::future_status::ready) {
      ChunkGeometryData geometryData = itGeo->get();
      ChunkComponent &chunk =
          m_registry.get<ChunkComponent>(geometryData.entity);

      // culled geometry
      updateChunkBuffers(chunk.geometryCulled, geometryData.indicesCulled,
                         geometryData.verticesCulled);

      // non-culled geometry
      updateChunkBuffers(chunk.geometryNonCulled, geometryData.indicesNonCulled,
                         geometryData.verticesNonCulled);

      // water geometry
      updateChunkBuffers(chunk.geometryTransparent,
                         geometryData.indicesTransparent,
                         geometryData.verticesTransparent);
...
ChunkCreateSystem::ChunkCreateSystem(Registry_T &registry,
  ...
  // event callbacks
  m_playerMovedHandle =
      EventDispatcher::onEntityMoved.subscribe([&](const EntityMovedEvent &e) {
        if (m_registry.any_of<PlayerComponent>(e.entity)) {
          handlePlayerMoved(e);
        }
      });

  handlePlayerMoved(EntityMovedEvent{nullptr, entt::null, glm::vec3{0.f},
                                     glm::vec3{FLT_MAX}});
}
