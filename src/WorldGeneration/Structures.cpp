...
std::unordered_map<StructureId, BlockCollection, StructureIdHash> Structures::structures = std::unordered_map<StructureId, BlockCollection, StructureIdHash>{};
...
BlockCollection Structures::getTreeBlocks(int height) {
	...
	for (int y = heightOffset; y <= height + 2; y++) {
		if (y < height + 1) {
			for (int x = -2; x <= 2; x++) {
				for (int z = -2; z <= 2; z++) {
