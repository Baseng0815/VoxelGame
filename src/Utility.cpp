...
bool inChunk(const glm::ivec3 &pos) {
  return pos.x >= 0 && pos.x < Configuration::CHUNK_SIZE && pos.y >= 0 &&
         pos.y < Configuration::CHUNK_HEIGHT && pos.z >= 0 &&
         pos.z < Configuration::CHUNK_SIZE;
}

bool inChunk(const int x, const int y, const int z) {
  return x >= 0 && x < Configuration::CHUNK_SIZE && y >= 0 &&
         y < Configuration::CHUNK_HEIGHT && z >= 0 &&
...
bool inChunk(const int x, const int y, const int z, const int chunkX,
             const int chunkZ) {
...
std::tuple<glm::ivec2, glm::ivec3>
GetChunkAndLocal(const glm::ivec3 &worldCoords) {
  glm::ivec2 chunkPos{
      std::floor(worldCoords.x / (float)Configuration::CHUNK_SIZE),
...
glm::ivec2 getChunk(const glm::ivec3 &worldCoords) {
  return glm::ivec2{
      (int)std::floor(worldCoords.x / (float)Configuration::CHUNK_SIZE),
      (int)std::floor(worldCoords.z / (float)Configuration::CHUNK_SIZE)};
}

glm::ivec3 getWorldCoords(const glm::ivec2 &chunk,
                          const glm::ivec3 &chunkCoords) {
  glm::ivec3 chunkOffset = glm::ivec3{chunk.x * Configuration::CHUNK_SIZE, 0,
                                      chunk.y * Configuration::CHUNK_SIZE};
  return chunkOffset + chunkCoords;
}

glm::ivec3 getWorldCoords(const int chunkX, const int chunkZ, const int x,
...
glm::ivec3 getLocalCoords(const glm::ivec3 &worldCoords) {
  int cx = worldCoords.x -
           std::floor(worldCoords.x / (float)Configuration::CHUNK_SIZE) *
               Configuration::CHUNK_SIZE;
  int cz = worldCoords.z -
           std::floor(worldCoords.z / (float)Configuration::CHUNK_SIZE) *
               Configuration::CHUNK_SIZE;
...
std::vector<glm::ivec3> getNeighborBlocks(const glm::ivec3 &blockPos,
                                          bool moore) {
...
glm::vec3 getBlockIntersectionFaceNormal(const glm::vec3 &blockPos,
                                         const Math::Ray &lookDirection) {
...
/// @brief
/// @param registry
/// @param inChunk chunk component of the current chunk
/// @param position position in chunk coordinates
/// @param action
void chunk_execute(
    entt::registry &registry, ChunkComponent &inChunk,
    const glm::ivec3 &position,
    std::function<void(ChunkComponent &, const glm::ivec3 &)> action) {
  if (position.y < 0 || position.y >= Configuration::CHUNK_HEIGHT) {
    return;
  }
  ...
  if (x < 0 || x >= Configuration::CHUNK_SIZE) {
    ...
    x = x - chunkXOff * Configuration::CHUNK_SIZE;
  }

  if (z < 0 || z >= Configuration::CHUNK_SIZE) {
    chunkZOff = floor(z / (float)Configuration::CHUNK_SIZE);
    z = z - chunkZOff * Configuration::CHUNK_SIZE;
  }

  if (chunkXOff != 0 || chunkZOff != 0) {
    ChunkComponent &other =
        World::getChunk(registry, glm::vec2{inChunk.chunkX + chunkXOff,
                                            inChunk.chunkZ + chunkZOff});
    action(other, glm::ivec3{x, y, z});
  } else {
    action(inChunk, position);
  }
}

void chunk_execute(
    entt::registry &registry, ChunkComponent &inChunk, const int &cx,
    const int &cy, const int &cz,
    std::function<void(ChunkComponent &, const int &, const int &, const int &)>
        action) {
