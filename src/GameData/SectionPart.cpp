#include "../../include/GameData/SectionPart.hpp"
...
SectionPart::SectionPart(const glm::ivec3& center, byte size, BlockId blockType)
...
void SectionPart::split() {
    ...
    for (int i = 0; i < SUBSECTIONS_COUNT; i++) {
        ...
        subSections[i] = SectionPart{newCenter, (byte)(size / 2), blockType};
    }
...
byte* SectionPart::getData(size_t* length) const {
    if (hasSubsections()) {
        byte* subSectionData[SUBSECTIONS_COUNT];
        ...
        *length = entireLength + 5;
        ...
        // write length
        for (int i = 0; i < 4; i++) {
            buffer[4 - i] = (byte)(entireLength >> (i * 8));
        }
        ...
        // write block type
        buffer[2] = (byte)((short)blockType >> 8);
        buffer[3] = (byte)blockType;
