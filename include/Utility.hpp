...
#define randNext(min, max) ((min + rand() % (max - min)))
...
namespace Utility {
    ...
    std::tuple<glm::ivec2, glm::ivec3> GetChunkAndLocal(const glm::ivec3& worldCoords);
    ...
    void chunk_execute(entt::registry& registry, ChunkComponent& chunk, const glm::ivec3& position, std::function<void(ChunkComponent&, const glm::ivec3&)> action);
    /* Executes an action at the given chunk position with bounds checking */
