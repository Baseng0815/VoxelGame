...
void main() {
    ...
    vec4 albedo = texture(gAlbedo, pass_uvCoords);    

    // 0.1 ^= ambient component
    vec3 lighting = albedo.rgb * 0.1;

    for (int i = 0; i < NR_LIGHTS; i++) {            
        vec3 diffuse = max(dot(normal, normalize(-lights[i].dir)), 0) * albedo.rgb * lights[i].color;
        lighting += diffuse;
    }

    out_Color = vec4(lighting, albedo.a);
}
